...
We're going to study the role that linear algebra plays in multiple regression model fitting. Let's load this $n$ = 100 row toy dataset. Note this data is not real, but simulated for pedagogical purposes.

```{r}
summary(toy_example)
#toy_example <- nrow(toy_example)
```

Let's define our linear model. $y$ : "inseam" as a function of $p$ = 3 predictor variables: weight, height, and age. 
...
```{r}
model_lm <- lm(model_formula, data = toy_example)
summary(model_lm)
```
...
Intuitively, you can think of the above results as follows: Once you know an individual's weight, there is no new information provided by height; they are completely redundant. We're now going to study the effects of this "multicollinearity" aka "redundancy" on our least squares estimates.
...
Recall that the design matrix `X1` was created from the original `toy_example`, but `X2` was created using perfectly collinear weight and height variables. 
...
**Final note**: If you have a predictor variable that is highly collinear with others, then at the very least, ["drop it like it's hot."](https://www.youtube.com/watch?v=RaCodgL9cvk){target="_blank"}

Acknowledgment: This tutorial was first implemented by Albert Kim (Smith College) and the code that generated the example data is from Prof. Randi Garcia
