...
We performed a logistic regression first to understand the probabilities of someone getting food stamps in Colorado. We wanted to use logistic regression  because it is easy to interpret for the sake of this research project, and it is a model we are very familiar with. Our food stamp variable FOODSTMP has a binary response, either 0, not receiving food stamps, or 1, receiving food stamps. Logistic regression works well with binary responses.It also has low variance because it assumes a linear relationship between the predictor and response.
